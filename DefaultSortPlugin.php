 ...
 * DefaultSort Plugin
 *
 * @copyright   Copyright 2014-2020 The Digital Ark, Corp.
 * @author	Anuragji
...
class DefaultSortPlugin extends Omeka_Plugin_AbstractPlugin {
	...
	protected $_filters = array('items_browse_default_sort', 'collections_browse_default_sort');
	...
	public function filterCollectionsBrowseDefaultSort($sort, $args) {
		...
		if (!is_admin_theme()) {
			...
			// Browse Collections
			if ($args['params']['controller'] == 'collections' && $args['params']['action'] == 'browse') {

				// Only apply the Default Sort if enabled and no other sort has been defined
				if (get_option('defaultsort_collections_enabled') && !isset($args['params']['sort_field'])) {

					$newSortField = get_option('defaultsort_collections_option');
					$newSortDir = get_option('defaultsort_collections_direction');
